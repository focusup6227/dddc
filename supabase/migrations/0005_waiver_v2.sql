-- Replace the seeded v1 waiver with the operator's real waiver text.
-- New row supersedes the old via the `active` flag; previously-signed
-- signatures stay attached to v1 (they reference waiver_id, not version).

update public.waivers set active = false where active = true;
